...
const validateDui = (value) => {
    if (value.length < 3) {
      return (
        <div className="alert alert-danger" role="alert">
          ¡El dui debe contener entre 3 y 50 caracteres!
        </div>
      );
    }
  };
...
const validateLastname = (value) => {
  if (value.length < 3 || value.length > 50) {
    return (
      <div className="alert alert-danger" role="alert">
        ¡El apellido debe contener entre 3 y 50 caracteres!
      </div>
    );
  }
};
...
class Register extends Component {
  constructor(props) {
    ...
    this.onChangeDui = this.onChangeDui.bind(this);
  ...
  onChangeDui(e) {
    this.setState({ dui: e.target.value, });
  }
  ...
  handleRegister(e) {
    ...
    if (this.checkBtn.context._errors.length === 0) {
      this.props
        .dispatch(
          register(this.state.dui, this.state.name, this.state.lastname, this.state.username, this.state.password)
        )
        .then(() => {
          this.setState({
            successful: true,
    ...
    return (
      ...
      <div className="col-md-12">
        <div className="card bg-light text-dark">
          <h1><center>Registro de cliente</center></h1>
          <Form onSubmit={this.handleRegister} ref={(c) => { this.form = c; }} >
            {!this.state.successful && (
              <div>
                <div className="form-group">
                  <label htmlFor="dui">Dui:</label>
                  <Input type="dui" className="form-control" name="dui" value={this.state.dui}
                    onChange={this.onChangeDui} validations={[required, validateDui]} />
                </div>
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <Input type="text" className="form-control" name="lastname" value={this.state.lastname}
                    onChange={this.onchangeLastname} validations={[required, validateLastname]} />
                </div>
                <div className="form-group">
